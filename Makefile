...
#
.PHONY: lint
lint: lint-action lint-yaml ## lint
...
release: ## release new version
	@select LEBEL in 'patch' 'minor' 'major'; do \
	  case "$${LEBEL}" in \
		'patch') gh workflow run _internal-release.yml -f level=patch; break ;; \
		'minor') gh workflow run _internal-release.yml -f level=minor; break ;; \
		'major') gh workflow run _internal-release.yml -f level=major; break ;; \
